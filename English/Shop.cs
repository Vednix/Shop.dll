...
namespace Shop
{
	...
	public class Shop : TerrariaPlugin
	{
		...
		private void Buy(CommandArgs args)
		{
			...
			{
				args.Player.SendErrorMessage("Check out our Shop, use: [c/ffd700:/buy name] or [c/ffd700:/shop menu]");
				return;
			}
			if (args.Parameters[0] == "menu")
			{
				...
				foreach (var gooda in config.All)
				{
					...
					{
						string perm = args.Player.Group.HasPermission(gooda.RequirePermission) ? "[c/84d184:(Available)]" : "[c/ff0000:(Shortage)]";
			...
			{
				args.Player.SendErrorMessage("Can't find a good with given name. Type [c/ffd700:/buy menu] for list.");
				return;
			}
			...
			{
				args.Player.SendErrorMessage($"The price of [c/ffa500:{Find.DisplayName}] is [c/50c878:{Money.Parse(Convert.ToString(Find.Price))}], but you only have [c/ff0000:{UsernameBankAccount.Balance}] in your account.");
				return;
			}
			...
			args.Player.SendSuccessMessage("You have paid [c/50c878{0}] to buy [c/ffa500:{1}].", amount2, Find.DisplayName);
